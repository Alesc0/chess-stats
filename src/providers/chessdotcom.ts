...
async function fetchRecentResults(username: string, limit = 10) {
  return traceAsync(
    "chessdotcom.fetchRecentResults",
    async () => {
      const now = new Date();
  ...
  return traceAsync(
    ...
    async () => {
      ...
      if (profileRes.status === 404)
        throw Object.assign(
          new Error(`Chess.com user "${username}" not found`),
          { status: 404 },
        );
...
export async function fetchChessDotComActivity(
  ...
  return traceAsync(
    ...
    async () => {
      ...
      const modeSet = mode
        ? new Set(mode.split(",").map((m) => m.trim()))
        : null;
