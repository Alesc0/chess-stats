...
const MODE_COLORS_FALLBACK: Record<string, string> = {
  bullet: "#f78166", blitz: "#ffa657", rapid: "#3fb950", puzzle: "#a371f7",
};
...
function formatDate(date: Date, monthsSpan: number): string {
  if (monthsSpan <= 3) return date.toLocaleDateString("en-US", { month: "short", day: "numeric" });
...
export function renderChart(opts: {
  ...
  platform: string;
  mode: string | string[];
  points: Array<{ date: Date; rating: number }> | Array<Array<{ date: Date; rating: number }>>;
  months?: number;
  themeName?: string;
  title?: string | null;
}): string {
  const { username, platform, mode, points, months = 6, themeName, title = null } = opts;
  const { colors: COLORS } = resolveTheme(themeName);

  const modes = Array.isArray(mode) ? mode : [mode];
  const series = Array.isArray(points) && Array.isArray(points[0]) ? points as any[][] : [points];

  const allSeries = modes
    .map((m, i) => ({ mode: m, points: (series[i] ?? []) as Array<{ date: Date; rating: number }> }))
    .filter((s) => s.points && s.points.length >= 2);

  const modeColor = (m: string) => (COLORS as any)[m] ?? MODE_COLORS_FALLBACK[m] ?? "#58a6ff";
  ...
  const seriesSVG = allSeries.map(({ mode: m, points: pts }) => {
    ...
    const polyPoints = pts.map((p) => `${toX(p.date.getTime()).toFixed(1)},${toY(p.rating).toFixed(1)}`).join(" ");
  ...
  const stars = renderStarEffect({ title, width: W, height: H, count: 12, clipId: "chartStarClip" });
  ...
  ${multiMode
      ? `<!-- Multi-mode legend -->${legendItems.join("")}`
