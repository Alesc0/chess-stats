...
export function svgToGroup(svgMarkup: string, opts: { id?: string; className?: string; viewBox?: string } = {}): string {
  ...
  const attrs = [id ? `id="${id}"` : "", className ? `class="${className}"` : "", transform]
    .filter(Boolean)
    .join(" ");
...
export function renderBlink(opts: {
  ...
  const { stats, username, platform, mode, points, months = 6, themeName } = opts;

  const cardSvg  = statsCard(stats, themeName);
  const chartSvg = renderChart({ username, platform, mode, points, months, themeName, title: stats.title ?? null });
